...
exports.add = (req, res) => {
  const { name, category, time, img_link } = req.body;
  let searchTerm = req.body.search;
  connection.query('INSERT INTO films SET name = ?, category = ?, time = ?, img_link = ?', [name, category, time, img_link], (err, rows) => {
    if (!err) {
...
exports.update = (req, res) => {
  const { name, category, time, img_link } = req.body;
  connection.query('UPDATE films SET name = ?, category = ?, time = ?, img_link = ? WHERE id = ?', [name, category, time, img_link, req.params.id], (err, rows) => {
